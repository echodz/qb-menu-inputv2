...
const OpenMenu = (data) => {
    ...
    $('.root-wrapper').addClass('active').fadeIn(0);
    $('.main-wrapper').fadeIn(0);
    $('.backgroundColour').fadeIn(0);

    let form = ["<form id='qb-input-form'>", `<div class="heading"><div class="heading-icon"></div>${data.header != null ? data.header : "Form Title"}</div>`];
...
const CloseMenu = () => {
    ...
    $('.backgroundColour').fadeOut(0);
    $('.root-wrapper').removeClass('active').fadeOut(0);
    formInputs = {};
};


const SetStyle = (style, themeColor) => {
    var stylesheet = $("<link>", {
        rel: "stylesheet",
        type: "text/css",
...
window.addEventListener("message", (event) => {
    ...
    switch (action) {
        case "SET_STYLE":
            return SetStyle(info, data.themeColor);
        case "OPEN_MENU":
